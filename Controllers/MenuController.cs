...
namespace cater_ease_api.Controllers
{
    ...
    public class MenuController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            var result = menus.Select(m => new MenuDetailDto
            {
                Id = m.Id,
                Name = m.Name,
                Description = m.Description,
                Dishes = allDishes.Where(d => m.DishIds.Contains(d.Id)).ToList(),
                Image = m.Image,
                Price = m.Price
            }).ToList();
        ...
        public async Task<IActionResult> Create([FromForm] CreateMenuDto form)
        {
            ...
            string imageUrl = await _cloudinary.UploadAsync(form.Image);
